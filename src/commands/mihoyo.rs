...
pub async fn genshin_codes(
    ...
    let codes_vec = extract_codes_from_string(codes);
    let no_of_codes = codes_vec.len();
    if no_of_codes < 1 {
        // TODO: Maybe should return an Error here? Idk...
        return Ok(())
    }
...
fn extract_codes_from_string(input: String) -> Vec<String> {
    let codes: Vec<String> = input.split(',').map(str::to_string).collect();
    codes.into_iter().filter(|x| x.len() == 12).collect::<Vec<String>>()
}
...
mod test {
    ...
    fn test_extract_codes() {
        ...
        let res1 = extract_codes_from_string(codes0);
        let res2 = extract_codes_from_string(codes1);
        let res3 = extract_codes_from_string(codes3);
        let res4 = extract_codes_from_string(codes4);
